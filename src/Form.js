...
import { isIgnoredField } from './utils';

const FormStyled = styled.form`
  width: ${props => props.theme.form.width};
  ${props => props.styledForm};
`;
...
export class Form extends Component {
  ...
  getValues = ({ name, forms, prefix }) => {
    ...
    return Object.keys(form).reduce((previous, key) => {
      ...
      if (form[key].fields) {
      ...
      } else if (isIgnoredField(ignore) || !form[key].value || form[key].value.trim() === '') {
        return previous;
        ...
        return {
          ...previous,
          [key]: type === 'date' ? new Date(form[key].value).toISOString() : form[key].value
        };
      }
    }, {});
  };
  ...
  onBlur = event => {
    ...
    const value = target.value === 'on' ? false : target.value === true ? true : target.value;
  ...
  onChange = event => {
    const { name, onChange, store, store: { forms, setFormFields } } = this.props;
    const target = event.target;
    const value = target.value === 'on' ? false : target.value === true ? true : target.value;
  ...
  onSubmit = event => {
    ...
    const { name, onSubmit, store: { forms, removeForm, setFormFields } } = this.props;
  ...
  validation = ({ form, target, prefix }) => {
    ...
    const errors = Object.keys(form).reduce((previous, key) => {
      ...
      if ((field.presence && !field.value) || (field.presence && field.value.trim() === '')) {
        return {
          ...previous,
          [targetField]: {
            label: field.label,
  ...
  render() {
    ...
    return (
      <FormStyled name={name} {...this.props} onSubmit={this.onSubmit}>
        {this.renderChildren()}
      </FormStyled>
    );
  }
